...
function SparkleField({ count, colorScheme }: { count: number; colorScheme: ColorScheme }) {
...
export function UnifiedBackground({
  ...
  const configs = {
    ...
    normal: { geometry: 8, particles: 300, sparkles: 25, size: 0.04 },
    dense: { geometry: 12, particles: 400, sparkles: 35, size: 0.05 },
  };

  const config =
    configs[variant as keyof typeof configs] ||
  ...
  return (
    <div className={containerClass}>
      <Canvas camera={{ position: [0, 0, 12], fov: 60 }} gl={{ antialias: true, alpha: true }}>
        ...
        <pointLight position={[-5, -5, 5]} intensity={0.3} color={lightColor} />
        ...
        {effects.includes('sparkles') && (
          <SparkleField count={config.sparkles} colorScheme={colorScheme} />
        )}
      </Canvas>
    </div>
  );
